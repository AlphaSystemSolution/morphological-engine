...
final class ConjugationBuilderHelper {
	...
	static AbbreviatedRecord createDefaultVerb(SarfTermType sarfTermType, VerbRootBase verbRootBase, RuleProcessor ruleProcessor,
		...
		final String value = verbTransformer.doTransform(ruleProcessor, verbRootBase.getRoot().getRootWord(), sarfTermType,
				outputFormat, rootLetters).getSingular();
		AbbreviatedRecord abbreviatedRecord = new AbbreviatedRecord();
		abbreviatedRecord.setId(verbRootBase.getId());
		abbreviatedRecord.setType(sarfTermType);
		abbreviatedRecord.setLabel(value);
		abbreviatedRecord.setSource(verbRootBase.getRoot().name());
		return abbreviatedRecord;
	}

	static AbbreviatedRecord createDefaultNoun(SarfTermType sarfTermType, NounRootBase nounRootBase, RuleProcessor ruleProcessor,
											   RootLetters rootLetters, OutputFormat outputFormat) {
		...
		final NounSupport singularBaseWord = nounRootBase.getSingularBaseWord();
		final RootWord rootWord = singularBaseWord.getRootWord();
		...
		final String value = SarfTermType.VERBAL_NOUN.equals(sarfTermType) ? nounConjugation.getAccusative() :
				nounConjugation.getNominative();
		AbbreviatedRecord abbreviatedRecord = new AbbreviatedRecord();
		abbreviatedRecord.setId(nounRootBase.getId());
		abbreviatedRecord.setType(sarfTermType);
		abbreviatedRecord.setLabel(value);
		abbreviatedRecord.setSource(singularBaseWord.name());
		return abbreviatedRecord;
	}
